# Simpleproject-os — o project-os que cabe num ESP32

> "seria legal até talvez ter integração com esp32, um Simpleproject-os, pra rodar
> direto de um esp32 pra qm quiser ou necessitar, com funcoes basicas obvio"

Um ESP32 tem **520 KB de SRAM** e nenhum sistema operacional. O project-os grande
...
página em `http://simpleproject-os.local` com os pinos, os sensores e uns horários.
Sem Raspberry Pi, sem nuvem, sem app.

**Satélite.** Você tem um Pi rodando o project-os. O ESP32 se anuncia no mDNS, o Pi
...
| Quem consegue contribuir | quem já mexe no project-os | quem tem toolchain |

O C++ ganha em memória e perde no que este projeto é. O project-os inteiro foi
...
simple/
  ...
  net/
    ...
    discovery.py     anúncio mDNS _project-os._tcp
    api.py           as rotas
  io/
...
Do lado do Pi, isso é um provedor a mais (`project_os_node`) e nenhuma tradução: as
...
Descoberta: `_project-os._tcp.local`, TXT com `id`, `name`, `fw`, `ents`. O
project-os grande já varre mDNS; um nó novo aparece como cartão de sugestão
("achei um Simpleproject-os chamado Gaiola, 2 saídas e 1 sensor — adotar?").
...
O Simpleproject-os é a ponta do sistema, não uma versão reduzida dele. A régua é:
se precisa de sistema de arquivos, é do Pi; se é um pino e um sensor, é daqui.

---

## 8. Gravar pelo próprio project-os

A integração que fecha o ciclo. Espeta o ESP32 na USB do Raspberry Pi:

1. O project-os vê a porta serial aparecer (`/dev/ttyUSB*`, `/dev/ttyACM*`) e
...
4. A placa reinicia, sobe o portal de Wi-Fi, e o próprio project-os já entrega as
...
Ordem: o project-os do Pi primeiro, inteiro e testado. Isto depois. Se você tiver um
