...
Este documento é o que o project-os precisa expor pro Argos ser bom aqui. Não é uma
...
project-os ele mora numa placa que não fecha, não dorme e reinicia sozinha via
...
Essa é a maior coisa que o project-os faz pelo Argos, e ela não custa uma linha de
...
- **`ARGOS_CONFIG_DIR`** já é lido do ambiente (`src/config.js`). Então o project-os
...
- **O PWA dele é o canal `pwa`**, com Web Push. Atrás do proxy do project-os ele
  ganha uma origem estável na LAN, que é do que um service worker precisa.

---

## 3. O canal `project_os` — o que o `echo` sempre quis ser
...
project-os. O venv Python já está lá, rodando, com os alto-falantes já descobertos.
...
Então o project-os oferece um alvo que um canal novo do Argos consome:
...
- **Sem se declarar indisponível, o canal some.** Se o project-os não responder ou
...
procura entidades `notify` da integração Alexa Devices; o project-os fala AirPlay
direto, via `pyatv`).
...
Por isso o project-os aplica o teto **também**, no servidor (`HOME.md` §7):
...
O project-os passa a ter `home.quiet_hours` no nível do sistema, e apps leem:
...
## 6. O que o project-os entrega, em lista
...
- **Não vou mexer no Argos.** Ele é do Miguel e está sendo escrito agora. O que
  está aqui é superfície do lado do project-os; o canal `project_os` no Argos é umas
...
- **Não vou duplicar o escalonamento.** O project-os notifica coisas do próprio
