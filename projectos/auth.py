...
opaque 32-byte URL-safe tokens carried by the ``projectos_session`` cookie or an
...
SALT_BYTES = 16
TOKEN_BYTES = 32
SESSION_COOKIE = "projectos_session"
...
def authenticate(db: Any, username: str, password: str) -> Optional[Dict[str, Any]]:
    ...
    if row is None:
        ...
        verify_password(password or "", hash_password("projectos-timing-equalizer"))
        return None
...
def _config(request: Request) -> Any:
    ...
    if config is not None:
        return config
    from projectos.config import get_config  # local import: avoids a cycle at import time
...
async def require_auth(request: Request) -> Dict[str, Any]:
    ...
    * no/expired session -> ``401 unauthenticated``
    ...
    if setup_required(db):
        raise ApiError(
            428, "setup_required", "ProjectOS has no user yet. Finish setup first."
        )
