...
Environment keys look like ``PROJECTOS__SERVER__PORT=8099``: the prefix is
...
DEFAULTS = {
    ...
    "apps": {
        "autostart": True,
        # Empty on purpose. A fresh ProjectOS has nothing installed; you open the
...
def env_overrides(environ: Optional[Mapping[str, str]] = None) -> Dict[str, Any]:
    """Build the nested override dict described by ``PROJECTOS__*`` variables."""
...
class Config(object):
    ...
    def persisted_dict(self) -> Dict[str, Any]:
        ...
        real decisions, and so upgrading ProjectOS can change a default.
    ...
    def save(self) -> Path:
        ...
        with self._lock:
            ...
            # any key someone also set through PROJECTOS__*.
            self._file = data
            self._rebuild()
            return target
