...
``PROJECTOS__SERVER__PORT`` silently winning over the value in the YAML is
otherwise indistinguishable from ProjectOS ignoring the setting.
"""
...
from projectos import auth, config as config_module
...
#: Settings ProjectOS refuses to write from the browser. ``file_roots`` widens
