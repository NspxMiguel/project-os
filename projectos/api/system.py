...
session **and** an explicit opt-in in the config, because ProjectOS is reachable
...
from projectos import __version__, auth
from projectos.core import hardware, sysinfo
from projectos.db import Database
from projectos.errors import ApiError
from projectos.main import get_config, get_db

log = logging.getLogger(__name__)

router = APIRouter(prefix="/system", tags=["system"])

#: Units ProjectOS is willing to talk about. An arbitrary unit name from the
#: browser is not accepted -- that would be a remote root shell with extra steps.
MANAGED_UNIT_PREFIXES = ("projectos", "home-assistant", "mosquitto", "zigbee2mqtt",
...
async def board(user: Dict[str, Any] = Depends(auth.require_auth)) -> Dict[str, Any]:
    ...
    :mod:`projectos.core.hardware` for why the gate is measured RAM rather than
...
async def services(
    ...
    """The systemd units ProjectOS knows about.
...
async def _control(
    ...
    if not any(stem.startswith(prefix) for prefix in MANAGED_UNIT_PREFIXES):
        raise ApiError(
            403, "unit_not_managed", "ProjectOS does not manage the unit %r." % unit
        )
    if not shutil.which("systemctl"):
