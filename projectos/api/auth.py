...
async def setup(
    ...
    if not auth.setup_required(db):
        raise ApiError(409, "already_configured", "ProjectOS already has a user.")
    ...
    log.info("ProjectOS claimed by user %r", user["username"])
...
async def login(
    ...
    if auth.setup_required(db):
        raise ApiError(428, "setup_required", "ProjectOS has no user yet.")
