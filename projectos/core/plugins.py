...
Apps are directories holding a ``manifest.json`` and Python code::
    ...
    ~/.projectos/apps/<id>/     user-installed, wins on id collision
    projectos/apps/<id>/        bundled with the release
...
from projectos.config import AppConfig, Config
...
def validate_manifest(data: Any, expected_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    manifest.setdefault("min_projectos", "0.0.0")
    ui = manifest.get("ui")
    ...
    required = _version_tuple(manifest["min_projectos"])
    if required > _version_tuple(__version__):
        raise ManifestError(
            "needs ProjectOS >= %s (this is %s)" % (manifest["min_projectos"], __version__)
        )
    return manifest
...
class AppContext(object):
    ...
    def __init__(
        ...
        self.logger = logger or logging.getLogger("projectos.app.%s" % app_id)
        self.config = config
        self.db = db
        self.bus = bus
...
class ContainerAppInstance(AppInstance):
    ...
    :mod:`projectos.core.containers` is the supervisor. Wrapping it in the
...
class PluginManager(object):
    ...
    def _import_entrypoint(self, record: LoadedApp) -> Any:
        ...
        if record.source == "builtin":
            module_name = "projectos.apps.%s.%s" % (record.id, module_part)
        ...
        package_name = "projectos_app_%s" % record.id.replace("-", "_")
    ...
    async def install_container(
        ...
        instance = ContainerAppInstance(
            app_id, engine, spec, data_dir, logger=logging.getLogger("projectos.app.%s" % app_id)
        )
        manifest = {
            "id": app_id,
