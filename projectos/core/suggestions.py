...
def rule_esp32_on_usb(ctx: Context) -> List[Dict[str, Any]]:
    """A microcontroller is plugged in. See docs/SIMPLEPROJECTOS.md section 8."""
    boards = [
    ...
    return [
        card(
            ...
            "Dá pra gravar o SimpleProjectOS nele: pinos, sensores e horários, "
            "controlados daqui ou sozinho. Sem abrir terminal.",
            icon="plug",
            priority=PRIORITY_HIGH,
            tags=["esp32", "simpleprojectos"],
...
def rule_adopt_node(ctx: Context) -> List[Dict[str, Any]]:
    """A SimpleProjectOS node is announcing itself on the LAN."""
    nodes = ctx.of_kind("projectos_node")
    if not nodes:
        return []
    return [
        card(
            "adopt-nodes",
            "%d nó SimpleProjectOS na rede" % len(nodes),
