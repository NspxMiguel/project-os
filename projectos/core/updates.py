"""Updating ProjectOS from ProjectOS.
...
Two ways in, picked automatically, because the two ways ProjectOS gets onto a
machine are different:

* **git** -- what ``install.sh`` does (a clone in ``/opt/projectos``). Updating is
...
3. **State is never touched.** ``PROJECTOS_HOME`` (database, config, media) lives
...
# ---------------------------------------------------------------------------
def root_dir() -> str:
    """The directory holding the ``projectos`` package -- the thing we replace."""
    import projectos

    package = os.path.dirname(os.path.abspath(projectos.__file__))
...
def _fetch_json(url: str, timeout: float = NETWORK_TIMEOUT) -> Dict[str, Any]:
    ...
    request = Request(url, headers={"User-Agent": "ProjectOS/%s" % __version__})
    try:
...
def _safe_extract(archive: str, into: str) -> str:
    ...
    if len(entries) == 1 and os.path.isdir(os.path.join(into, entries[0])):
        # GitHub-style tarballs wrap everything in ProjectOS-1.2.3/.
        return os.path.join(into, entries[0])
    return into


def _looks_like_projectos(path: str) -> bool:
    return os.path.isdir(os.path.join(path, "projectos")) and os.path.isfile(
        os.path.join(path, "projectos", "__init__.py")
    )
...
def apply_tarball(info: Dict[str, Any], root: Optional[str] = None,
    ...
    workdir = tempfile.mkdtemp(prefix=".projectos-update-", dir=parent)
    try:
        ...
        if not _looks_like_projectos(extracted):
            raise UpdateError(
                "The archive does not contain a ProjectOS tree.", code="bad_archive",
            )
...
def install_requirements(root: Optional[str] = None, on_line: Optional[Any] = None) -> int:
    ...
    everywhere in ProjectOS by design.
    """
    where = root or root_dir()
    say = on_line or (lambda line: None)
...
def restart(on_line: Optional[Any] = None) -> str:
    ...
    if under_systemd() and shutil.which("systemctl"):
        say("restarting the projectos service")
        subprocess.Popen(["systemctl", "restart", "projectos"])
        return "systemd"
    argv = restart_argv()
...
def restart_argv(argv: Optional[List[str]] = None, executable: Optional[str] = None) -> List[str]:
    ...
    ``python -m projectos`` leaves ``sys.argv[0]`` as the path to
    ``projectos/__main__.py``. Re-executing *that* path is not the same command:
    Python then puts ``projectos/`` itself on sys.path instead of its parent, so
    ``import projectos`` fails unless the package also happens to be installed in
    ...
    if os.path.basename(first) == "__main__.py":
        package = os.path.basename(os.path.dirname(os.path.abspath(first))) or "projectos"
