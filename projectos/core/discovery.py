...
ProjectOS in, and it already knows there is an Apple TV in the living room and a
...
microcontroller  serial_port, flashable    (extension, docs/SIMPLEPROJECTOS.md)
===============  ==========================================================
...
from projectos.db import utcnow_iso
...
#: docs/SIMPLEPROJECTOS.md). They never replace a normative capability.
SWITCH = "switch"
LIGHT = "light"
...
KIND_CAPABILITIES = {
    ...
    # so you can see it and name it; ProjectOS claims nothing about it.
    "phone": (),
    "smart_plug": (SWITCH,),
    "smart_light": (LIGHT,),
    "tuya_device": (),
...
#: (docs/SIMPLEPROJECTOS.md section 8). (vendor, product) -> human name.
USB_SERIAL_IDS = {
...
def scan_serial() -> List[Observation]:
    ...
    board" and offer to flash SimpleProjectOS onto it.
    """
    found = []  # type: List[Observation]
...
#: Scanners that live in :mod:`projectos.core.lan`, named rather than imported.
...
def run_extra_scanners() -> List[Observation]:
    ...
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=len(scanners), thread_name_prefix="projectos-scan"
    ) as pool:
