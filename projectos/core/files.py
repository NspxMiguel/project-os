...
attack surface in ProjectOS, so it is built around a single rule:
...
The roots are ``PROJECTOS_HOME`` plus whatever ``security.file_roots`` adds.
...
def _outside(message: str = "", detail: Any = None) -> ApiError:
    return ApiError(
        403,
        "path_outside_root",
        message or "That path is outside the folders ProjectOS is allowed to touch.",
        detail,
    )
...
def _config(config: Any = None) -> Any:
    if config is not None:
        return config
    from projectos.config import get_config  # local import: avoids an import cycle
...
def _configured_extra_roots(config: Any) -> List[Path]:
    try:
        ...
        log.warning("security.file_roots is unreadable; falling back to PROJECTOS_HOME only")
        return []
...
def root_paths(config: Any = None) -> List[Path]:
    ...
    try:
        ...
        raise ApiError(500, "no_home", "PROJECTOS_HOME is not usable: %s" % exc)
    out = [first]
    seen = {str(first)}
        ...
        out.append(
            {
                ...
                "name": "ProjectOS" if is_home else (path.name or str(path)),
                "path": str(path),
                "is_home": is_home,
                "exists": exists,
...
def resolve(path: Any = "", config: Any = None, follow_final: bool = True) -> Path:
    ...
    Relative paths are relative to ``PROJECTOS_HOME``; absolute paths are
    ...
    Raises :class:`~projectos.errors.ApiError` -- ``invalid_path`` (400) or
