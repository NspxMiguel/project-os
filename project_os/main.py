...
from project_os.config import Config, load_config, set_config
from project_os.core.discovery import DeviceRegistry
from project_os.core.plugins import PluginManager
from project_os.core.suggestions import SuggestionEngine
from project_os.db import Database
from project_os.errors import ApiError, install_error_handlers
from project_os.events import EventBus

log = logging.getLogger("project_os")
...
def _tagged(handler: logging.Handler) -> logging.Handler:
    setattr(handler, "_project_os_handler", True)
    return handler


def _remove_our_handlers(root: logging.Logger) -> None:
    for handler in list(root.handlers):
        if getattr(handler, "_project_os_handler", False):
...
def configure_logging(config: Config) -> None:
    """stderr + a rotating file under PROJECT_OS_HOME. Idempotent."""
...
def _resolve_stats_callable() -> Callable[[], Any]:
    ...
    for name in _STATS_FUNCTION_NAMES:
        ...
        if callable(candidate):
            ...
            return candidate
    log.warning("project_os.core.sysinfo exposes no stats function")
    _stats_callable = _fallback_stats
    return _stats_callable
...
async def lifespan(app: FastAPI):
    ...
    log.info("project-os %s starting (home=%s)", __version__, paths.home())
    ...
    try:
        ...
        log.info("project-os shutting down")
        await _cancel(tasks)
        with contextlib.suppress(Exception):
...
def create_app() -> FastAPI:
    """Build the project-os application."""
    app = FastAPI(
        title="project-os",
        version=__version__,
    ...
    try:
        from project_os.api import api_router
    ...
    async def health(request: Request) -> JSONResponse:
        ...
        pending = None
        if db is not None:
            from project_os.auth import setup_required
    ...
    if web_root.is_dir():
        ...
        async def no_frontend() -> PlainTextResponse:
            return PlainTextResponse(
                "project-os %s is running, but the web/ directory was not found at %s.\n"
