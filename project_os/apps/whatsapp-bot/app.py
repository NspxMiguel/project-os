...
from project_os.config import is_redacted
...
def register_builtin_commands(registry: CommandRegistry) -> None:
    ...
    def cmd_apps(context: Any) -> str:
        ...
        from project_os.main import get_plugins  # local import: avoids a hard import cycle at module load
        from project_os.core.plugins import STATE_RUNNING
...
def build_router(instance: "WhatsAppBotApp") -> APIRouter:
    ...
    # design: Meta (or a bridge) cannot present a project-os session cookie, so
...
class WhatsAppBotApp(AppInstance):
    def __init__(self, ctx: AppContext) -> None:
        self.ctx = ctx
        self.log = getattr(ctx, "logger", None) or logging.getLogger("project_os.apps.whatsapp-bot")
