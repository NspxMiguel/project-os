...
:class:`~project_os.core.plugins.PluginManager` to load it. Progress is
...
from project_os.core.plugins import APP_ID_RE, ManifestError, validate_manifest
...
def _urlopen_bytes(url: str, limit: int, timeout: float) -> bytes:
    ...
    request = urllib.request.Request(
        url, headers={"User-Agent": "project-os/%s" % __version__, "Accept": "*/*"}
    )
    try:
            ...
            try:
                async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
                    async with client.stream(
                        "GET", url, headers={"User-Agent": "project-os/%s" % __version__}
                    ) as response:
...
class AppStore(object):
    ...
    def _normalise_entry(self, raw: Any, repo: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        entry = {
            ...
            "min_project_os": str(raw.get("min_project_os") or "0.0.0"),
    ...
    async def catalog(self, refresh: bool = False) -> Dict[str, Any]:
        ...
        for app_id, info in installed.items():
            ...
            merged[app_id] = {
                ...
                "min_project_os": str(info["manifest"].get("min_project_os") or "0.0.0"),
        ...
        return {
            ...
            "project_os_version": __version__,
        }
    ...
    def _annotate(self, entry: Dict[str, Any], installed: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        compatible = compare_versions(item.get("min_project_os", "0.0.0"), __version__) <= 0
        item["compatible"] = compatible
        item["incompatible_reason"] = (
            None
            if compatible
            else "Needs project-os %s or newer (this is %s)." % (item["min_project_os"], __version__)
        )
    ...
    def _guard_target(self, app_id: str, installed: Dict[str, Dict[str, Any]], update: bool) -> bool:
        ...
        if self.is_bundled(app_id) and not update:
            raise ApiError(
                409,
                "app_id_conflict",
                "%r is the id of an app that ships with project-os. Installing it would "
