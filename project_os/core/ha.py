...
project-os is not built on Home Assistant and does not need it. But a lot of
houses already have one, and when they do it is the cheapest way to reach a
hundred devices project-os would otherwise have to speak to one protocol at a
...
class HomeAssistantClient(object):
    ...
    Built either from the project-os config (the normal path, so changing the
