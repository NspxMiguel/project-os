...
store's job (:mod:`project_os.api.store`), and the split matters: after the
concept change a clean project-os has nothing installed at all, so this endpoint
...
owned by :mod:`project_os.main`, which guards it against path traversal.
"""
...
from project_os.main import get_db, get_plugins
